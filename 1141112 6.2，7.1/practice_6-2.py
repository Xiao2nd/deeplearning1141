...
testPredictPlot[test_start:test_start + len(trainPredict), :] = testPredict


# Show the actual closing price and the predicted closing price for the last three trading days
print("Shows the actual closing price and the predicted closing price for the last three trading days\n")

# Get the last 3 predictions from test data
last_3_actual = testY_actual[-3:]
last_3_predict = testPredict[-3:]

# Get the corresponding dates
test_start_index = train_size + look_back
last_3_dates = df2.index[test_start_index + len(testY_actual) - 3:test_start_index + len(testY_actual)]

#last 3 days' actual and predicted prices
for i in range(3):
    print(f"Date: {last_3_dates[i].date()}, Actual: {last_3_actual[i][0]:.2f}, Predicted: {last_3_predict[i][0]:.2f}")
